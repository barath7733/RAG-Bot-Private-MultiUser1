...
// ---------------------------- Auth guard ----------------------------
// The page itself is already protected server-side (main.py redirects
// unauthenticated visitors to /login before this HTML is ever sent).
// This just handles the case where a session expires/is revoked while
// the tab stays open: any 401 from an API call bounces back to /login.
...
// ---------------------------- Markdown rendering ----------------------------
// The LLM returns markdown (**bold**, tables, lists, etc). This converts it
// to safe HTML for display instead of showing the raw ** and | symbols.

function renderMarkdown(text) {
  if (!text) return "";

  // Escape HTML first so user/LLM text can't inject markup, then re-apply markdown.
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith("|") && lines[i + 1] && /^\s*\|?[\s:-]+\|[\s:|-]*$/.test(lines[i + 1])) {
      ...
      headerCells.forEach((c) => (tableHtml += `<th>${inlineMarkdown(c)}</th>`));
      tableHtml += "</tr></thead><tbody>";
      i += 2;
      while (i < lines.length && lines[i].trim().startsWith("|")) {
        const rowCells = lines[i].trim().replace(/^\||\|$/g, "").split("|").map((c) => c.trim());
        tableHtml += "<tr>";
        rowCells.forEach((c) => (tableHtml += `<td>${inlineMarkdown(c)}</td>`));
        tableHtml += "</tr>";
        i++;
      }
    ...
    if (headingMatch) {
      const level = Math.min(headingMatch[1].length + 2, 6); // keep headings modest in-chat
      htmlParts.push(`<h${level} class="md-heading">${inlineMarkdown(headingMatch[2])}</h${level}>`);
      i++;
      continue;
    }
    ...
    if (/^\s*[-*]\s+/.test(line)) {
      ...
      while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) {
        listHtml += `<li>${inlineMarkdown(lines[i].replace(/^\s*[-*]\s+/, ""))}</li>`;
        i++;
      }
      listHtml += "</ul>";
    ...
    if (/^\s*\d+[.)]\s+/.test(line)) {
      ...
      while (i < lines.length && /^\s*\d+[.)]\s+/.test(lines[i])) {
        listHtml += `<li>${inlineMarkdown(lines[i].replace(/^\s*\d+[.)]\s+/, ""))}</li>`;
        i++;
      }
      listHtml += "</ol>";
      htmlParts.push(listHtml);
    ...
    htmlParts.push(`<p class="md-p">${inlineMarkdown(line)}</p>`);
    i++;
  }
...
el.attachImageBtn.addEventListener("click", () => el.visionFileInput.click());

el.visionFileInput.addEventListener("change", () => {
  ...
  if (file.size > 10 * 1024 * 1024) {
    alert("Image exceeds the maximum allowed size of 10 MB.");
    el.visionFileInput.value = "";
    return;
  }

  state.pendingImage = file;
  el.imageAttachThumb.src = URL.createObjectURL(file);
  el.imageAttachName.textContent = file.name;
  el.imageAttachPreview.classList.remove("hidden");
  el.chatInput.placeholder = "Ask something about this image (optional)...";
  el.chatInput.focus();
});
...
function clearPendingImage() {
  ...
  el.visionFileInput.value = "";
  el.imageAttachPreview.classList.add("hidden");
  el.imageAttachThumb.src = "";
  el.chatInput.placeholder = "Ask a question, or attach an image to ask about it...";
}

el.imageAttachRemove.addEventListener("click", clearPendingImage);
...
async function submitImageAnalysis() {
  ...
  clearWelcomeMessage();
  appendMessage({ role: "user", content: question || "🖼️ (image attached)" });
  state.history.push({ role: "user", content: question || "[Attached an image]" });
  ...
  try {
    const response = await fetch(API.visionAnalyze, { method: "POST", body: formData, credentials: "same-origin" });
    if (handleUnauthorized(response)) return;
    const data = await response.json();

    if (!response.ok) {
      throw new Error(data.error || data.detail || "Image analysis failed.");
    }

    state.sessionId = data.session_id;
    appendMessage({ role: "assistant", content: data.answer, modeUsed: "vision" });
    state.history.push({ role: "assistant", content: data.answer });
    loadChatHistory();
  } catch (err) {
    appendMessage({ role: "assistant", content: `Something went wrong: ${err.message}`, isError: true });
  } finally {
    setLoading(false);
...
// ---------------------------- Chat history (server-side, per-user) ----------------------------

async function loadChatHistory() {
  try {
    const response = await fetch(API.chatHistory, { credentials: "same-origin" });
    if (handleUnauthorized(response)) return;
...
function renderHistoryList() {
  if (state.sessions.length === 0) {
    el.historyList.innerHTML = `<p class="empty-hint">No chats yet.</p>`;
    return;
  }
  ...
  state.sessions.forEach((session) => {
    const item = document.createElement("div");
    item.className = "history-item" + (session.session_id === state.sessionId ? " active" : "");
    item.innerHTML = `
      <span class="history-title" title="${escapeHtml(session.title)}">${escapeHtml(session.title)}</span>
      <button class="delete-btn" data-id="${session.session_id}" title="Delete chat">🗑️</button>
    `;
    item.querySelector(".history-title").addEventListener("click", () => openChatSession(session.session_id));
...
async function openChatSession(sessionId) {
  try {
    const response = await fetch(`${API.chatHistory}/${sessionId}`, { credentials: "same-origin" });
    if (handleUnauthorized(response)) return;
    if (!response.ok) throw new Error("Could not load that chat.");
    const detail = await response.json();

    state.sessionId = detail.session_id;
    state.history = detail.messages.map((m) => ({ role: m.role, content: m.content }));
    clearWelcomeMessage();
    el.chatWindow.innerHTML = "";
    detail.messages.forEach((m) => {
      appendMessage({ role: m.role, content: m.content, modeUsed: m.mode_used });
    });
    renderHistoryList();
  } catch (err) {
    console.error(err);
...
async function deleteChatSession(sessionId) {
  try {
    const response = await fetch(`${API.chatHistory}/${sessionId}`, { method: "DELETE", credentials: "same-origin" });
    if (handleUnauthorized(response)) return;
    if (!response.ok) throw new Error("Could not delete that chat.");
    if (state.sessionId === sessionId) startNewChat();
...
async function deleteDocument(documentId) {
  try {
    const response = await fetch(`${API.documents}/${documentId}`, { method: "DELETE", credentials: "same-origin" });
    if (handleUnauthorized(response)) return;
    const data = await response.json();
    if (!response.ok) {
...
el.imageForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const promptChanged = data.final_prompt && data.final_prompt !== data.prompt;
    wrapper.innerHTML = `
      ...
      <div class="bubble generated-image-wrapper">
        ...
        ${data.note ? `<div class="image-limitation-note">⚠️ ${escapeHtml(data.note)}</div>` : ""}
        ${promptChanged ? `<div class="image-enhanced-note">✨ Enhanced prompt used: ${escapeHtml(data.final_prompt)}</div>` : ""}
      </div>
    `;
    el.chatWindow.appendChild(wrapper);
    el.chatWindow.scrollTop = el.chatWindow.scrollHeight;
