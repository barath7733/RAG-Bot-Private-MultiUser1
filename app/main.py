...
RAG + web), document upload/list/delete, chat history, vision (image
analysis), and a health check. Serves the frontend from /static and
/templates.
...
from app import groq_client, rag
...
app = FastAPI(
    ...
    description=(
        "A multi-user chatbot that answers general questions via Groq "
        "and grounds document-related questions in retrieved context "
        "using per-user Pinecone vector search."
    ),
    version="2.0.0",
)
...
async def on_startup() -> None:
    ...
    logger.info(
        "Application startup complete. Groq model=%s, "
        "Embedding dimension configured, DB=%s",
        settings.groq_model,
...
async def register(
    ...
    token, _ = create_session_and_token(
        db,
        user,
    )
...
async def chat(
    ...
    session = rag.get_or_create_session(
        db,
        user.id,
        payload.session_id,
        payload.question,
...
async def analyze_uploaded_image(
    ...
    try:
        ...
        answer = groq_client.analyze_image(
            data_url,
            question or None,
        )
...
async def get_chat_history(
    ...
    detail = rag.get_chat_session_detail(
        db,
        user.id,
        session_id,
    )
...
async def delete_chat_history(
    ...
    deleted = rag.delete_chat_session(
        db,
        user.id,
        session_id,
    )
...
async def upload_document(
    ...
    safe_name = Path(
        file.filename
        or f"document-{uuid.uuid4().hex[:8]}.pdf"
    ).name
    ...
    return UploadResponse(
        document=doc_info,
        message=(
            f"'{safe_name}' processed and indexed successfully."
        ),
    )
